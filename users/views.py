...
def userFollowUnfollow(request,pk=None):
    ...
    if other_user not in current_user.profile.friends.all():
        current_user.profile.friends.add(other_user)
        other_user.profile.friends.add(current_user)
        
        notify = Notification.objects.create(sender=current_user,receiver=other_user,action="and you are now frinds.")

    else:
        current_user.profile.friends.remove(other_user)
        other_user.profile.friends.remove(current_user)
